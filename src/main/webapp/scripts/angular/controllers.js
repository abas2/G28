...
agoraUSControllers.controller('MainController', [ '$scope', '$route',
		...
		function($scope, $route, $routeParams, $location) {
			...
			$scope.dataHasLoaded = true;// Hay que ponerlo al final para que
										// angular cargue la vista despues de la
...
agoraUSControllers.controller('visualizacionController', [
		'$http',
		'$scope',
		'$routeParams',
		function($http, $scope, $routeParams) {
			...
			$scope.params = $routeParams;
			$http.get(
					"api/resultados/encuestas.do?encuesta="
							+ $routeParams.encuesta).then(
...
agoraUSControllers.controller('mapaController', [ '$http', '$scope',
		'$routeParams', function($http, $scope, $routeParams) {
			// $scope.dataHasLoaded=false;
			showHeaderAndFooter($scope, "Inicio");
...
agoraUSControllers.controller('preguntasController', [
		'$http',
		'$scope',
		'$routeParams',
		function($http, $scope, $routeParams) {
			...
			$scope.params = $routeParams;
			$http.get("api/resultados/preguntas/votadas.do").then(
					function successCallback(response) {
						try {
							console.log("Cargado");
			...
			$scope.calculaTotalVotos = function(idEncuesta, idPregunta){
			     var sum = 0;

			     for(var i = 0 ; i < $scope.encuestas[idEncuesta].preguntas[idPregunta].opcions.length ; i++){
			        sum = sum + $scope.encuestas[idEncuesta].preguntas[idPregunta].opcions[i].recuento;
			     }
